...
func _ready():
	assert(_logs)
	_logs.info("Splash scene > started.")
	assert(_state)
	state_dict = _state.get_dict(state_name)
	...
	if init_script == null:
		init_script = load("res://addons/GameBackend/scripts/init_template.gd").new()
...
func on_init_complited():
	_logs.info("Splash scene > init complited.")
